...
# Vercel deployments cannot write to the source directory. SQLite is therefore
# placed in /tmp when running on Vercel. This keeps the demo functional; for
# durable production data, replace SQLite with a hosted database.
DB_PATH = os.environ.get(
    "SCAMX_DB_PATH",
    "/tmp/scamx.db" if os.environ.get("VERCEL") else os.path.join(os.path.dirname(__file__), "scamx.db")
)
